...
for (let i = 0; i < rucksacks.length; i++) {
    ...
    for(let j = 0; j<secondCompartment.length;j++){
        ...
        if(occurances[secondCompartment[j]][0]>0){
            prioritySum += occurances[secondCompartment[j]][2]
            break;
        }
    }
    occurances = JSON.parse(JSON.stringify(originalOccurances));
}
